...
int main()
{
	...
	for (int x = 0; x < amount; x++)
	{
		/*for (int z = 0; z < amount; z++)
		{
			...
			{
				//std::shared_ptr<Entity> physicsTest = app->CreateEntity(glm::vec3(25 + (-15 * x) + rand() % 5, 50 + (15) + rand() % 5, -30 + (-15) + rand() % 5), glm::vec3(rand() % 360, rand() % 360, rand() % 360), glm::vec3(1,1,1));
	...
	for (int x = 0; x < amount; x++)
	{
		std::shared_ptr<Entity> physicsTest = app->CreateEntity(glm::vec3(25, 50, -30), glm::vec3(0, 0, 0), glm::vec3(1, 1, 1));
	}
